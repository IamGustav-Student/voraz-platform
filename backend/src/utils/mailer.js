 ...
 *   devToken está presente únicamente cuando SMTP no está configurado (modo dev).
 */
export const sendPasswordResetEmail = async ({ to, resetUrl, brandName = 'GastroRed', rawToken = null }) => {
  const t = getTransporter();

  if (!t) {
    // Modo desarrollo: logueamos el link y lo devolvemos para que el frontend lo muestre
    console.log('\n[MAILER DEV] Se hubiera enviado email de recuperación:');
    console.log(`  → Para: ${to}`);
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#111;border-radius:16px;overflow:hidden;border:1px solid #222;">
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <div style="text-align:center;margin:32px 0;">
                <a href="${resetUrl}" 
                   style="display:inline-block;background:#E30613;color:#fff;text-decoration:none;padding:14px 36px;border-radius:10px;font-size:16px;font-weight:800;letter-spacing:0.5px;transition:all 0.2s;">
              ...
              <p style="margin:8px 0 0;word-break:break-all;">
                <a href="${resetUrl}" style="color:#E30613;font-size:12px;font-family:monospace;">${resetUrl}</a>
              </p>
            </td>
          </tr>
          <!-- Footer -->
          <tr>
            <td style="padding:20px 40px;border-top:1px solid #222;text-align:center;">
              <p style="margin:0;color:#444;font-size:12px;">© ${new Date().getFullYear()} ${brandName} — GastroRed Platform</p>
            </td>
          </tr>
        </table>
      </td>
