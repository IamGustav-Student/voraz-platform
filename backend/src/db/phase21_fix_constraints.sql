...
-- [NUEVO] Eliminamos la constraint si ya existe para evitar errores en re-despliegues
ALTER TABLE categories DROP CONSTRAINT IF EXISTS categories_slug_store_id_unique CASCADE;
...
-- [NUEVO] Eliminamos la constraint si ya existe para evitar errores en re-despliegues
ALTER TABLE coupons DROP CONSTRAINT IF EXISTS coupons_code_store_id_unique CASCADE;
