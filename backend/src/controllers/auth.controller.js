...
const generateToken = (user) =>
    jwt.sign({ id: user.id, email: user.email, name: user.name, role: user.role || 'user', store_id: user.store_id }, JWT_SECRET, { expiresIn: TOKEN_EXPIRY });

export const register = async (req, res) => {
    const { email, password, name, phone } = req.body;
    const storeId = req.store?.id || 1;
    const storeName = req.store?.brand_name || 'la plataforma';
    if (!email || !password || !name) return res.status(400).json({ status: 'error', message: 'Email, contraseña y nombre son requeridos.' });
    try {
        const existing = await query('SELECT id FROM users WHERE email = $1 AND store_id = $2', [email, storeId]);
...
export const login = async (req, res) => {
    ...
    try {
        const result = await query('SELECT * FROM users WHERE email = $1 AND store_id = $2', [email, storeId]);
...
export const me = async (req, res) => {
    try {
        const result = await query(
            'SELECT id, email, name, phone, avatar_url, points, role, store_id, created_at FROM users WHERE id = $1',
            [req.user.id]
        );
...
export const googleCallback = async (req, res) => {
    ...
    try {
        const existing = await query('SELECT id, email, name, phone, points, role, store_id FROM users WHERE google_id = $1 OR email = $2', [google_id, email]);
        let user;
